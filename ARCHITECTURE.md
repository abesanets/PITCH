...
* `installer.iss` — Inno Setup script to generate single-file Windows installer (`PITCH_Setup_v1.51.exe`).
...
* `core/__init__.py` — Exports version constant (`__version__ = "1.51"`) and core package symbols (`PitchCore`, `STTWorker`, `ClipboardManager`, `AudioRecorder`, `LogRedirector`).
