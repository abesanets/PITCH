...
FORMATTING_PRESETS = {
    "default": (
        ...
        "8. Переводи русскоязычный технический сленг и названия технологий в правильное английское написание: 'экзешник' -> '.exe', 'питон' -> 'Python', 'джэсон' -> 'JSON', 'гитхаб' -> 'GitHub', 'хтмл' -> 'HTML', 'цсс' -> 'CSS', 'пдф' -> 'PDF', 'ютуб' -> 'YouTube', 'мсп' -> 'MCP', 'деплой' -> 'деплой'.\n"
        "9. Форматируй числа и числительные логично. По умолчанию используй цифры вместо слов (например, '5' вместо 'пять'), особенно для количеств, дат, номеров. Порядковые числительные записывай цифрами с буквенным наращением через дефис, строго соблюдая падежные окончания и род (правила русского языка): 'первый' -> '1-й', 'второго' -> '2-го', 'третьему' -> '3-му', 'десятые' -> '10-е'. Если число используется в устойчивом выражении или фразеологизме, его можно оставить словами."
    ),
    "chat": (
    ...
    "translate_en": (
        ...
        "9. Переводи технический сленг и названия технологий в правильное английское написание: 'питон' -> 'Python', 'гитхаб' -> 'GitHub', 'мсп' -> 'MCP', 'деплой' -> 'deploy' и т.д.\n"
    ...
    for model in models:
        try:
            ...
            transcription = client.audio.transcriptions.create(
                ...
                model=model,
                language="ru",
                prompt="питон, python, питоне, экзешник, exe, скрипт, код, ЛЛМ, LLM, cmd, windows, ctrl, alt, shift, файл, джэсон, json, гитхаб, github, репозиторий, хтмл, html, цсс, css, пдф, pdf, MCP, mcp, МСП, мсп, deploy, деплой, деплоить, задеплоить, деплоя, деплоймент, deployment, хостинг"
            )
